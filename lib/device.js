...
function Device(nodeName, name, type, accessModel, publishModel, transducers) {
	if (arguments.length === 2) {
		return this.initWithClient(nodeName, name);
	}

	return this.init(nodeName, name, type, accessModel, publishModel, transducers);
}
...
Device.prototype.init = function (nodeName, name, type, accessModel, publishModel, transducers) {
	...
	*/
	this.transducers = transducers ? $.extend(true, {}, transducers) : [];
...
/**
* @param nodeName XMPP node name of this device. required
* @param client connected sox client.
*/
Device.prototype.initWithClient = function (nodeName, client) {
	...
	* runtime information that won't be included in meta data
	*/
	this.transducers = []; // array of transducers
...
Device.fromJson = function (jsonObject) {
	var input = jsonObject.device;
	var device = new Device();
	device.name = input.name;
	device.type = input.type;
	device.nodeName = input.nodeName ? input.nodeName : input.name;

	for (var i = 0; i < jsonObject.device.transducers.length; i++) {
		var transducer = Transducer.fromJson(input.transducers[i]);
