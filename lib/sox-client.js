...
SoxClient.prototype.disconnect = function () {
	...
	if (!this.isAnonymousAllowed && !this.authenticated) {
		return false;
	}
...
SoxClient.prototype.resolveDevice = function (device) {
	...
	this.connection.PubSub.items(device.nodeName + '_meta')
...
SoxClient.prototype.deleteDevice = function (device) {
	...
	var successMetaCallback = function () {
		/**
	...
	var successDataCallback = function () {
		console.log('[SoxClient.js] Deleted: ' + device.nodeName + '_data');
		me.connection.PubSub.deleteNode(device.nodeName + '_meta')
			.done(successMetaCallback)
			.fail(failureCallback);
	};
